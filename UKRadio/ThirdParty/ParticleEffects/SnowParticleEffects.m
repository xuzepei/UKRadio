...
@implementation SnowParticleEffects

+ (void)show
{
    UIWindow *window = [UIApplication sharedApplication].delegate.window;

    UIView *backgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, window.bounds.size.width, window.bounds.size.height * 0.5)];
...
CAEmitterLayer *addEmitterLayer(UIView *view)
{
    ...
    subCell3.name = @"blue";
    CAEmitterCell *subCell4 = subCell([UIImage imageNamed:@"star"]);
    subCell4.name = @"star";
    
    CAEmitterLayer *emitterLayer = [CAEmitterLayer layer];
    emitterLayer.emitterPosition = CGPointMake(view.bounds.size.width * 0.5, 0); //控制发射器位置
    emitterLayer.emitterSize	= CGSizeMake(50, 50);//控制发射器区域大小
    emitterLayer.emitterMode	= kCAEmitterLayerOutline;
    emitterLayer.emitterShape	= kCAEmitterLayerRectangle;
    emitterLayer.renderMode		= kCAEmitterLayerOldestFirst;
    //emitterLayer.speed = 5;
